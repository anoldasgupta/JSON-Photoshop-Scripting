...
<javascriptresource>
<name>Adjustment &amp; Fill Gallery...</name>
<about>"Adjustment &amp; Fill Gallery" v2.3

Apply a preset of adjustement layers and fill layers on an existing image.

Creative script using the "JSON Action Manager" scripting library.
© 2013-2016 Michel MARIANI.
</about>
<menu>automate</menu>
<category>JSON Action Manager Color Effects</category>
...
//------------------------------------------------------------------------------
// File: Adjustment & Fill Gallery.js
// Version: 2.3
// Release Date: 2016-04-10
// Copyright: © 2013-2016 Michel MARIANI <http://www.tonton-pixel.com/blog/>
...
//  - Used new version 4.2 of jamLayers scripting library module.
//  1.8:
//  - Used new version 4.0 of scripting library modules.
//	1.7:
//  - Improved code.
//  - Added Psychedelic Art preset.
//	1.6:
//  - Used new version 3.6.7 of jamLayers scripting library module.
//	1.5:
//  - Added Lomo Photography preset.
//	1.4:
//  - Used new version 3.6.6 of jamLayers scripting library module.
//  - Improved Color Infrared preset.
//	1.3:
//  - Changed effects menu item text from "*All Effects*" to "*Every Effect*".
//  - Used new version 3.6.5 of jamLayers scripting library module.
//	1.2:
//  - Allowed stop or continue when JSON file error.
//	1.1:
//	- Fixed bug:
//    no item selected in effect menu when running the script for the first time.
//	1.0:
//	- Initial release.
//------------------------------------------------------------------------------
...
var signature = "json-action-manager-adjustment-and-fill-gallery-options";
var defaultOptions =
{
	desiredSet: "~",
	desiredEffect: "~",
	mergeDuplicate: false,
	createLayerSet: false
};
...
{
	var dialog = new Window ('dialog', "Adjustment & Fill Gallery");
	...
	optionsPanel.alignment = [ "fill", "middle" ];
	optionsPanel.orientation = "row";
	optionsPanel.alignChildren = [ "fill", "middle" ];
	dialog.mergeDuplicateCheckbox = optionsPanel.add ('checkbox', undefined, "Merge Duplicate");
	dialog.mergeDuplicateCheckbox.value = customOptions.mergeDuplicate;
	dialog.createLayerSetCheckbox = optionsPanel.add ('checkbox', undefined, "Create Layer Set");
	dialog.createLayerSetCheckbox.value = customOptions.createLayerSet;
	...
	{
		customOptions.desiredSet = dialog.setMenu.selection.text;
		customOptions.desiredEffect = dialog.effectMenu.selection.text;
		customOptions.mergeDuplicate = dialog.mergeDuplicateCheckbox.value;
		customOptions.createLayerSet = dialog.createLayerSetCheckbox.value;
		dialog.close (true);
	};
	dialog.onShow = function ()
	{
		dialog.setMenu.selection = dialog.setMenu.find (customOptions.desiredSet) || 0;
		dialog.effectMenu.selection = dialog.effectMenu.find (customOptions.desiredEffect) || 0;
	};
	dialog.center ();
	return dialog.show ();
}
...
{
	if (!customOptions.mergeDuplicate)
	{
		var hasBackground = true;
		try
		{
			jamEngine.jsonGet ([ { "backgroundLayer": { "<property>": "background" } } ]);
		}
		catch (e)
		{
			hasBackground = false;
		...
		if (hasBackground)
		{
			...
			(
				"select",
				{
					"target": { "<reference>": [ { "backgroundLayer": { "<property>": "background" } } ] },
		...
		else
		{
			...
			(
				"select",
				{
					"target": { "<reference>": [ { "layer": { "<index>": 1 } } ] },
					"makeVisible": { "<boolean>": false }
				}
			);
			jamEngine.jsonPlay
		...
		var placeholderLayerId = resultDescriptorObj["layerID"]["<integer>"];
		jamEngine.jsonPlay
		(
			"move",
			{
				"target": { "<reference>": [ { "layer": { "<enumerated>": { "ordinal": "targetEnum" } } } ] },
				"to": { "<reference>": [ { "layer": { "<enumerated>": { "ordinal": "front" } } } ] }
			}
		);
		jamEngine.jsonPlay
		(
	...
	if (customOptions.createLayerSet)
	{
		jamEngine.jsonPlay
		(
			"make",
			{
				...
				"using": { "<object>": { "layerSection": { "name": { "<string>": setEffectName } } } }
			}
		);
	}
	var layerObj = effects[desiredSet][desiredEffect];
	for (var layerIndex = 0; layerIndex < layerObj.length; layerIndex++) 
	{
		try
		{
			jamLayers.makeLayer (layerObj[layerIndex]);
		}
		catch (e)
		{
		}
	...
	if (!customOptions.mergeDuplicate)
	{
		...
		(
			"delete",
			{
				"target": { "<reference>": [ { "layer": { "<identifier>": placeholderLayerId } } ] }
			}
		);
	}
}
...
function processEffect (desiredSet, desiredEffect)
{
	...
	var duplicateDesc = 
	{
		...
		"merged": { "<boolean>": customOptions.mergeDuplicate }
	};
	if (nameDocument)
	{
			...
			for (var i = 0; i < jsonFiles.length; i++)
			{
				...
				if (leafName[0] !== '~') // Skip files whose name begins with a tilde character
				{
					...
					if (setName && setName[1])
					{
						try
						{
							var effectSet = jamUtils.readJsonFile (jsonFile);
							var effectCount = 0;
			...
			if (!stop)
			{
				...
				if (colorSpace === "RGBColor")
				{
					...
					if (getParameters ())
					{
						...
						if (allEffectsInMenu && (customOptions.desiredEffect === allEffects))
						{
							resultDescriptorObj = jamEngine.jsonGet
							(
								[
							...
							var docIds = [ ];
							for (var k in effects[customOptions.desiredSet])
							{
								if (effects[customOptions.desiredSet].hasOwnProperty (k))
								{
									jamEngine.jsonPlay
									(
										"select",
									...
									processEffect (customOptions.desiredSet, k);
									resultDescriptorObj = jamEngine.jsonGet
									(
										[
											{ "property": { "<property>": "documentID" } },
						...
						{
							processEffect (customOptions.desiredSet, customOptions.desiredEffect);
						}
					}
				}
				else
