...
<javascriptresource>
<name>Change Layer Content...</name>
<about>"Change Layer Content" v1.0

Change the current layer content (fill layer or adjustment layer).
...
function getParameters ()
{
	...
	var frameHeight = 2;
	var frameYs = [ ];
	var dialog = new Window ('dialog', "Change Layer Content", [ 0, 0, dialogWidth, dialogHeight ]);
	dialog.panel = dialog.add ('panel', [ dialogPadding[0], dialogPadding[1], dialogWidth - dialogPadding[0], dialogHeight - dialogPadding[1] ], 'Layer Content');
	dialog.panel.buttons = [ ];
	var y = panelPadding[1] + extraGap;
	for (var k in layerContent)
	{
		if (layerContent.hasOwnProperty (k))
		{
			if (appVersion >= layerContent[k]["minVersion"])
			{
				if (k !== "-")
				{
					dialog.panel.buttons.push (dialog.panel.add ('radiobutton', [ panelPadding[0], y, (dialog.panel.bounds.right - dialog.panel.bounds.left) - panelPadding[0], y + buttonHeight ], k));
				...
				else
				{
					y += buttonGap;
					frameYs.push (y);
					y += frameHeight + buttonGap + buttonGap;
				}
			}
		}
	}
	for (var i = 0; i < frameYs.length; i++)
	{
		dialog.panel.add ('panel', [ panelPadding[0], frameYs[i], (dialog.panel.bounds.right - dialog.panel.bounds.left) - panelPadding[0], frameYs[i] + frameHeight ]);
	}
	var count = dialog.panel.buttons.length;
	for (var i = 0; i < count; i++)
	{
		var radbut = dialog.panel.buttons[i];
		radbut.value = false;
		radbut.enabled = (radbut.text !== currentContent);
		radbut.onClick = function () { desiredContent = this.text; };
	}
	for (var i = 0; i < count; i++)
	{
		var radbut = dialog.panel.buttons[i];
		if (layerContent[radbut.text]["layer"] === layerContent[currentContent]["layer"])
		{
			if (radbut.enabled)
			{
				radbut.value = true;
				desiredContent = radbut.text;
				break;
			}
		}
	}
	dialog.panel.bounds.bottom = dialog.panel.bounds.top + y + panelPadding[1];
	dialog.bounds.bottom = dialog.bounds.top + dialogPadding[1] + (dialog.panel.bounds.bottom - dialog.panel.bounds.top) + dialogPadding[1] + buttonHeight + dialogPadding[1];
	dialog.cancelBtn = dialog.add ('button', [ dialogPadding[0], dialog.bounds.bottom - dialogPadding[1] - buttonHeight, dialogPadding[0] + buttonWidth, dialog.bounds.bottom - dialogPadding[1] ], 'Cancel', { name: "cancel" });
	dialog.cancelBtn.onClick = function () { this.parent.close (false); };
	dialog.OKBtn = dialog.add ('button', [ dialog.bounds.right - dialogPadding[0] - buttonWidth, dialog.bounds.bottom - dialogPadding[1] - buttonHeight, dialog.bounds.right - dialogPadding[0], dialog.bounds.bottom - dialogPadding[1] ], 'OK', { name: "ok" });
	dialog.OKBtn.onClick = function () { this.parent.close (true); };
	dialog.center ();
	return dialog.show ();
}
...
function changeContent (desiredContent)
{
	try
	{
		jamEngine.jsonPlay
		(
			"set",
			{
				"target": [ "<reference>", [ [ layerContent[desiredContent]["layer"], [ "<enumerated>", [ "ordinal", "targetEnum" ] ] ] ] ],
				"to": [ "<class>", layerContent[desiredContent]["type"] ]
			},
			DialogModes.ALL
		);
	}
...
function getCurrentContent ()
{
	...
	try
	{
		...
		(
			[
				[ "property", [ "<property>", "adjustment" ] ],
				[ "layer", [ "<enumerated>", [ "ordinal", "targetEnum" ] ] ]
			]
		);
		currentType = resultDescriptorObj["adjustment"][1][0][1][0];
	}
	catch (e)
	{
		alert ("Cannot change the content of the current layer.\rIt must be either a fill layer or an adjustment layer.");
	}
	if (currentType)
	{
		for (var k in layerContent)
		{
			if (layerContent.hasOwnProperty (k))
			{
				if ((k !== "-") && (layerContent[k]["type"] === currentType))
				{
					currentContent = k;
					break;
				}
...
{
	getCurrentContent ();
	if (currentContent && getParameters ())
	{
		changeContent (desiredContent);
	}
}
