...
<javascriptresource>
<name>Full Mirror...</name>
<about>"Full Mirror" v4.0

Apply a "full-size" symmetry effect on an existing image, including possible transparency.

Creative script using the "JSON Action Manager" scripting library.
© 2011-2015 Michel MARIANI.
</about>
<menu>automate</menu>
<category>JSON Action Manager Mirror Effects</category>
...
var effects =
{
	"Top Left": { x: 20, y: 20, name: 'ok', hLoc: "left", vLoc: "top" },
	"Top": { x: 140, y: 20, name: '', hLoc: "center", vLoc: "top" },
	"Top Right": { x: 260, y: 20, name: '', hLoc: "right", vLoc: "top" },
	"Left": { x: 20, y: 60, name: '', hLoc: "left", vLoc: "center" },
	"Cancel": { x: 140, y: 60, name: 'cancel' },
	"Right": { x: 260, y: 60, name: '', hLoc: "right", vLoc: "center" },
	"Bottom Left": { x: 20, y: 100, name: '', hLoc: "left", vLoc: "bottomEnum" },
	"Bottom": { x: 140, y: 100, name: '', hLoc: "center", vLoc: "bottomEnum" },
	"Bottom Right": { x: 260, y: 100, name: '', hLoc: "right", vLoc: "bottomEnum" }
};
var desiredEffect = "Top Left";

var useSuspendHistory = (parseInt (app.version) >= 10);	// CS3
var nameDocument = !useSuspendHistory;

//------------------------------------------------------------------------------

function getParameters ()
{
	var dlg = new Window ('dialog', "Full Mirror", [ 0, 0, dialogWidth, dialogHeight ]);
	dlg.buttons = [];
	for (var k in effects)
	{
	    if (effects.hasOwnProperty (k))
	    {
			...
			dlg.buttons.push (dlg.add ('button', [ effect.x, effect.y, effect.x + buttonWidth, effect.y + buttonHeight ], k, (effect.name != '') ? { name: effect.name } : undefined));
		}
	}
	var count = dlg.buttons.length;
	for (var i = 0; i < count; i++)
	{
		var button = dlg.buttons[i];
		button.onClick = function () { desiredEffect = this.text; this.parent.close (this.text != "Cancel"); };
	}
	dlg.center ();
	return dlg.show ();
}
...
function fullMirror (effect, layerName)
{
	var isOpaque = true;
	try
	{
		jamEngine.jsonGet ([ { "layer": { "<property>": "background" } } ]);
	}
	catch (e)
	{
		isOpaque = false;
	...
	if (isOpaque)
	{
		...
		(
			"set",
			{
				...
				"to": { "<class>": "layer" }	// "to": { "<object>": { "layer": null } }
			}
		);
	}
	var width = (effect.hLoc != "center") ? 100 : 0;
	var height = (effect.vLoc != "center") ? 100 : 0;
	jamEngine.jsonPlay
	(
		"canvasSize",
		{
			"relative": { "<boolean>" : true },
			"width": { "<unitDouble>": { "percentUnit": width } },
			"height": { "<unitDouble>": { "percentUnit": height } },
			"horizontal": { "<enumerated>": { "horizontalLocation": effect.hLoc } },
			"vertical": { "<enumerated>": { "verticalLocation": effect.vLoc } },
			"canvasExtensionColorType": { "<enumerated>": { "canvasExtensionColorType": "white" } }
		}
	);
	jamEngine.jsonPlay
	(
		"select",
		{
			"target": { "<reference>": [ { "menuItemClass": { "<enumerated>": { "menuItemType": "fitOnScreen" } } } ] }
		}
	);
	if (width != 0)
	{
		...
		jamEngine.jsonPlay	// Select All
		(
			"set",
			{
				"target": { "<reference>": [ { "channel": { "<property>": "selection" } } ] },
				"to": { "<enumerated>": { "ordinal": "allEnum" } }
			}
		);
		jamEngine.jsonPlay
		(
			...
			{
				"target": { "<reference>": [ { "layer": { "<enumerated>": { "ordinal": "targetEnum" } } } ] },
				"axis": { "<enumerated>": { "orientation": "horizontal" } }
			}
		);
		...
		jamEngine.jsonPlay ("mergeLayers");	// Merge down
	}
	if (height != 0)
	{
		jamEngine.jsonPlay
		...
		(
			...
			{
				"target": { "<reference>": [ { "layer": { "<enumerated>": { "ordinal": "targetEnum" } } } ] },
				"axis": { "<enumerated>": { "orientation": "vertical" } }
			}
		);
		jamEngine.jsonPlay	// Deselect
		(
	...
	if (isOpaque)
	{
		jamEngine.jsonPlay ("flattenImage");
	}
	else
	{
		jamEngine.jsonPlay
		(
			...
			{
				"target": { "<reference>": [ { "layer": { "<enumerated>": { "ordinal": "targetEnum" } } } ] },
				"to": { "<object>": { "layer": { "name": { "<string>": layerName } } } }
			}
		);
	}
}
...
resultDescriptorObj = jamEngine.jsonGet ([ { "property": { "<property>": "numberOfDocuments" } }, { "application": { "<enumerated>": { "ordinal": "targetEnum" } } } ]);
var docCount = resultDescriptorObj["numberOfDocuments"]["<integer>"];
if (docCount > 0)
{
	if (getParameters ())
	{
		var effect = effects[desiredEffect];
		var effectName = "Full Mirror" + " | " + desiredEffect;
		var duplicateDesc = 
		{
		...
		if (nameDocument)
		{
			duplicateDesc["name"] = { "<string>": effectName };
		}
		jamEngine.jsonPlay ("duplicate", duplicateDesc);
		if (useSuspendHistory)
		{
			app.activeDocument.suspendHistory (effectName, "fullMirror (effect, effectName);");
		}
		else
		{
			fullMirror (effect, effectName);
		}
	}
}
