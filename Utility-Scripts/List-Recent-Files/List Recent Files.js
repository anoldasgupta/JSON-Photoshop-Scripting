...
<javascriptresource>
<name>List Recent Files...</name>
<about>"List Recent Files" v4.0

List in JSON format the full paths of all the recently opened files in Photoshop (same as "File &gt; Open Recent").
...
var promptString = "List recent files to JSON file:";

var signature = "json-action-manager-list-recent-files-options";

var defaultOptions =
{
	defaultFile: "~/Desktop/recent-files-list-ps" + appVersion + ".json",
	dialogWidth: 720,
	dialogHeight: 640,
	dialogX: 0,
	dialogY: 0
...
function displayDialog ()
{
	...
	var dialog = new Window ('dialog', "List Recent Files", undefined, { resizeable: true });
...
if (appVersion >= 10)	// CS3 or later
{
	// List all recent files in JSON format
	var recentFilesArr = [ ];
	var recentFileName;
	if (useDOM)
	{
		...
		{
			recentFileName = recentFiles[i].fsName;
			if (recentFileName !== "")
			{
				recentFilesArr.push (recentFileName);
			}
		}
	}
	else
	{
		...
		(
			[
				{ "property": { "<property>": "recentFiles" } },
				{ "application": { "<enumerated>": { "ordinal": "targetEnum" } } }
			]
		);
		if ("recentFiles" in resultDescriptorObj)
		{
			var recentFilesList = resultDescriptorObj["recentFiles"]["<list>"];
			for (var i = 0; i < recentFilesList.length; i++)
			{
				recentFileName = recentFilesList[i]["<path>"];
				if (recentFileName !== "")
				{
					recentFilesArr.push (recentFileName);
				}
