...
<javascriptresource>
<name>Parse JavaScript Code...</name>
<about>"Parse JavaScript Code" v1.3

Parse JavaScript code to an abstract syntax tree (AST) or to a list of tokens.

Utility script using the "JSON Action Manager" scripting library.
© 2014-2015 Michel MARIANI.
</about>
<menu>automate</menu>
<category>JSON Action Manager Syntax Utility</category>
</javascriptresource>

*/

//------------------------------------------------------------------------------
// File: Parse JavaScript Code.js
// Version: 1.3
...
//  1.1:
//  - Renamed include file "esprima.js" to "esprima.jsxinc" to avoid being
//    displayed in Photoshop Scripts menu.
...
//@include "esprima.jsxinc"

//------------------------------------------------------------------------------

var signature = "json-action-manager-parse-javascript-code-options";

var defaultCode =
	"// Life, Universe, and Everything\r" +
	"var answer = 6 * 7;";

var defaultOptions =
{
	resultDefaultFile: "~/Desktop/parsed-javascript-code.json",
	code: defaultCode,
	tokenize: false,
	dialog:
	{
...
function parseJavaScriptCode (code, tokenize)
{
	var parsedCode;
	try
	{
		parsedCode = (tokenize) ? esprima.tokenize (code) : esprima.parse (code);
	}
	catch (e)
	{
		parsedCode = "Error: Line " + e.lineNumber + ": " + e.message;
	}
	return parsedCode;
}
...
function displayDialog ()
{
	...
	var dialog = new Window ('dialog', "Parse JavaScript Code", undefined, { resizeable: true });
	...
	dialog.onShow = function ()
	{
		...
		tokenizeCheck.value = customOptions.tokenize;
	...
	codeGroup.alignment = [ "fill", "top" ];
	var codeArea = codeGroup.add ('edittext', undefined, "\r\r\r\r\r\r\r\r\r\r\r\r\r\r\r", { multiline: true });
	...
	var codeOptionsGroup = codeGroup.add ('group');
	var tokenizeCheck = codeOptionsGroup.add ('checkbox', undefined, 'Tokenize');
	var expandTabsCheck = codeOptionsGroup.add ('checkbox', undefined, 'Expand Tabs');
	var parseButton = codeGroup.add ('button', undefined, 'Parse');
	parseButton.alignment = [ "center", "center" ];
	parseButton.onClick = function ()
	{
		var parsedCode = parseJavaScriptCode (codeArea.text, tokenizeCheck.value);
		parsedCodeArea.text =
			(typeof parsedCode === 'string') ? parsedCode : jamJSON.stringify (parsedCode, expandTabsCheck.value ? 4 : '\t');
		saveResultButton.enabled = !(typeof parsedCode === 'string');
	};
	var parsedCodeArea = dialog.add ('edittext', undefined, "", { multiline: true, readonly: true });
	parsedCodeArea.alignment = [ "fill", "fill" ];
	parsedCodeArea.minimumSize = [ -1, 120 ];
	if (monospacedFont)
	{
		parsedCodeArea.graphics.font = monospacedFont;
	}
	var file;
	var saveResultButton = dialog.add ('button', undefined, 'Save...');
	saveResultButton.alignment = [ "center", "bottom" ];
	saveResultButton.onClick = function ()
	{
		file = resultDefaultFile.saveDlg ("Save JSON text to file:");
		if (file)
		{
			jamUtils.writeTextFile (file, parsedCodeArea.text);
			if (openWithEditor)
			{
				file.execute ();
			}
	...
	okButton.onClick = function ()
	{
		...
		customOptions.tokenize = tokenizeCheck.value;
