...
<javascriptresource>
<name>List Fonts...</name>
<about>"List Fonts" v4.1

List in JSON format all fonts currently available in Photoshop.

Utility script using the "JSON Action Manager" scripting library.
© 2011-2015 Michel MARIANI.
</about>
<menu>automate</menu>
<category>JSON Action Manager Fonts Utility</category>
</javascriptresource>
...
var promptString = "List fonts to JSON file:";

var signature = "json-action-manager-list-fonts-options";

var defaultOptions =
{
	defaultFile: "~/Desktop/fonts-list-ps" + appVersion + ".json",
	dialogWidth: 720,
	dialogHeight: 640,
	dialogX: 0,
	dialogY: 0
...
function displayDialog ()
{
	...
	var dialog = new Window ('dialog', "List Fonts", undefined, { resizeable: true });
...
else
{
	...
	var fontListObj = resultDescriptorObj["fontList"]["<object>"]["fontList"];
	var fontNameArr = fontListObj["fontName"]["<list>"];
	var fontPostScriptNameArr = fontListObj["fontPostScriptName"]["<list>"];
	var fontFamilyNameArr = fontListObj["fontFamilyName"]["<list>"];
	var fontStyleNameArr = fontListObj["fontStyleName"]["<list>"];
	for (var i = 0; i < fontNameArr.length; i++)
	{
		var family = fontFamilyNameArr[i]["<string>"];
		if (!(family in fontsObj))
		{
			fontsObj[family] = { };
		}
		fontsObj[family][fontStyleNameArr[i]["<string>"]] =
		{
			"name": fontNameArr[i]["<string>"],
			"postScriptName": fontPostScriptNameArr[i]["<string>"]
		};
	}
}
//
...
if (appVersion === 8)	// CS
{
	if ((!defaultFile.exists) || confirm ("The file « " + File.decode (defaultFile.name) + " » already exists. Do you want to overwrite it?"))
	{
		...
		alert ("Fonts list file created:\r" + defaultFile.fsName);
	}
}
