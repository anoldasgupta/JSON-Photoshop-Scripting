...
<javascriptresource>
<name>Get Document Info Code...</name>
<about>"Get Document Info Code" v1.6

Get information in JSON AM Data Format from the currently active document.

Utility script using the "JSON Action Manager" scripting library.
© 2013-2017 Michel MARIANI.
</about>
<menu>automate</menu>
...
//------------------------------------------------------------------------------
// File: Get Document Info Code.js
// Version: 1.6
// Release Date: 2017-03-30
// Copyright: © 2013-2017 Michel MARIANI <http://www.tonton-pixel.com/blog/>
...
var signature = "json-action-manager-get-document-info-code-options";

var defaultOptions =
{
	defaultFile: "~/Desktop/get-document-info.js",
	dialogWidth: 512,
	dialogHeight: 768,
	dialogX: 0,
	dialogY: 0
...
var getLayersInfo = false;
var getChannelsInfo = false;
var getPathsInfo = false;
var getHistoryStatesInfo = false;
...
function displayDialog (text)
{
	...
	var dialog = new Window ('dialog', "Get Document Info Code", undefined, { resizeable: true });
...
if (appVersion >= 10)	// CS3
{
	...
	try
	{
		ref = [ { "document": { "<enumerated>": { "ordinal": "targetEnum" } } } ];
	...
	if (desc)
	{
		var layerCount = desc["numberOfLayers"]["<integer>"];		// Does not include the background layer
		var channelCount = desc["numberOfChannels"]["<integer>"];	// Includes color channels and alpha channels
		var pathCount = desc["numberOfPaths"]["<integer>"];			// Includes normal paths, the work path and the vector mask
		//
		var documentInfoCode =
			codeHeader +
			"\n" +
			...
			"\n";
		// Document
		documentInfoCode += getInfoCode (ref, desc, "Get properties of current document");
		if (getLayersInfo)
		{
			// Background layer
			desc = null;
			try
			{
				ref =
				[
					{ "backgroundLayer": { "<property>": "background" } },
			...
			if (desc)
			{
				documentInfoCode += getInfoCode (ref, desc, "Get properties of background layer of current document");
			}
			// Layers
			for (var layerIndex = 0; layerIndex < layerCount; layerIndex++)
			{
				...
				desc = jamEngine.jsonGet (ref);
				documentInfoCode += getInfoCode (ref, desc, "Get properties of layer " + (layerIndex + 1) + " of current document");
			}
		}
		if (getChannelsInfo)
		{
			...
			for (var channelIndex = 0; channelIndex < channelCount; channelIndex++)
			{
				ref =
				[
					{ "channel": { "<index>": channelIndex + 1 } },
					{ "document": { "<enumerated>": { "ordinal": "targetEnum" } } }
				];
				desc = jamEngine.jsonGet (ref);
				documentInfoCode += getInfoCode (ref, desc, "Get properties of channel " + (channelIndex + 1) + " of current document");
			}
		}
		if (getPathsInfo)
		{
			...
			for (var pathIndex = 0; pathIndex < pathCount; pathIndex++)
			{
				...
				if (desc)
				{
					documentInfoCode += getInfoCode (ref, desc, "Get properties of path " + (pathIndex + 1));
				}
			}
			// Work path
			desc = null;
			...
			if (desc)
			{
				documentInfoCode += getInfoCode (ref, desc, "Get properties of work path");
			}
			// Vector mask
			desc = null;
			try
			{
				ref = [ { "path": { "<enumerated>": { "path": "vectorMask" } } } ];
				desc = jamEngine.jsonGet (ref);
			}
			catch (e)
			{
			...
			if (desc)
			{
				documentInfoCode += getInfoCode (ref, desc, "Get properties of vector mask");
			}
		}
		if (getHistoryStatesInfo)
		{
			// History states
			desc = null;
			ref = [ { "historyState": { "<enumerated>": { "ordinal": "targetEnum" } } } ];
			desc = jamEngine.jsonGet (ref);
			var historyStateCount = desc["count"]["<integer>"];	// Includes snapshots and history states
			for (var historyStateIndex = 0; historyStateIndex < historyStateCount; historyStateIndex++)
			{
				desc = null;
				try
				{
					ref = [ { "historyState": { "<index>": historyStateIndex + 1 } } ];
					desc = jamEngine.jsonGet (ref);
				}
				catch (e)
				{
				...
				if (desc)
				{
					documentInfoCode += getInfoCode (ref, desc, "Get properties of history state " + (historyStateIndex + 1));
				}
			}
		}
		//
		var customOptions = jamUtils.getCustomOptions (signature, defaultOptions);
		if (displayDialog (documentInfoCode))
		{
