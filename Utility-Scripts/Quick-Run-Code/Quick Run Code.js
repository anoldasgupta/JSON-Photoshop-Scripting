...
var headers =
{
	...
	//
	"JSON Action Manager with Options":
		'//@includepath "~/JSON Action Manager/"\n' +
		'//@include "jamActions.jsxinc"\n' +
		'//@include "jamBooks.jsxinc"\n' +
		'//@include "jamColors.jsxinc"\n' +
...
var defaultOptions =
{
	"defaultFile": "~/Desktop/quick-run-code.js",
	"headerPreset": "None",
...
function displayDialog ()
{
	...
	var dialog = new Window ('dialog', "Quick Run Code", undefined, { resizeable: true });
	...
	headerGroup.add ('statictext', undefined, "Header:");
	var headerArr = [ ];
	...
	headerMenu.helpTip = "Select a predefined header";	// "Select a predefined or custom header"
...
//			return f.name.match (/\.jsx?$/i) || (f.type === 'TEXT');
		...
		fileIn = defaultFile.openDlg ("Load code from file:", jsFilter);
		if (fileIn)
		{
	...
	{
		fileOut = defaultFile.saveDlg ("Save code to file:");
		if (fileOut)
		{
	...
	var dontRunButton = buttonsGroup.add ('button', undefined, 'Don’t Run');
	...
	var runButton = buttonsGroup.add ('button', undefined, 'Run', { name: "ok" });
...
if (appVersion >= 11)	// CS4
{
	...
	if (displayDialog ())
	{
		...
		if (doRun)
		{
			if (useTempFile)
			{
				var tempFile = new File (Folder.temp + "/quick_run_code.js");
