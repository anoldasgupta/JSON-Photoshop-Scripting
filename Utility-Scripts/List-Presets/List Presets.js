...
<javascriptresource>
<name>List Presets...</name>
<about>"List Presets" v4.1

List in JSON format the names of all presets (brushes, swatches, gradients, styles, patterns, contours, custom shapes, and tools) currently available in Photoshop.
...
var promptString = "List presets to JSON file:";

var signature = "json-action-manager-list-presets-options";

var defaultOptions =
{
	defaultFile: "~/Desktop/presets-list-ps" + appVersion + ".json",
	dialogWidth: 720,
	dialogHeight: 640,
	dialogX: 0,
	dialogY: 0
...
function displayDialog ()
{
	...
	var dialog = new Window ('dialog', "List Presets", undefined, { resizeable: true });
...
var categories =
{
	"brush": "brushes",
	"color": "swatches",
	"gradientClassEvent": "gradients",
	"styleClass": "styles",
	"'PttR'": "patterns",
	"shapingCurve": "contours",
...
if (appVersion === 8)	// CS
{
	if ((!defaultFile.exists) || confirm ("The file « " + File.decode (defaultFile.name) + " » already exists. Do you want to overwrite it?"))
	{
		...
		alert ("Presets list file created:\r" + defaultFile.fsName);
	}
}
