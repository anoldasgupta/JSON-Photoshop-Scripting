...
<javascriptresource>
<name>List Internal IDs...</name>
<about>"List Internal IDs" v4.1

List in JSON format all internal IDs used by the Photoshop’s programming interface.
...
var promptString = "List internal ids to JSON file:";

var signature = "json-action-manager-list-internal-ids-options";

var defaultOptions =
{
	defaultFile: "~/Desktop/internal-ids-list-ps" + appVersion + ".json",
	dialogWidth: 720,
	dialogHeight: 640,
	dialogX: 0,
	dialogY: 0
...
var listRuntimeIds = false;
var listPredefinedIds = true;

var listReverseToo = true;
...
function displayDialog ()
{
	...
	var dialog = new Window ('dialog', "List Internal IDs", undefined, { resizeable: true });
	...
	saveButton.onClick = function ()
	{
		...
		if (file)
		{
			jamUtils.writeTextFile (file, logString);
			defaultFile = file;
		}
	};
...
if (confirm ("Proceed with the listing of all Photoshop internal IDs?\nWarning: it could definitely take some time..."))
{
	...
	{
		var runtimeIds = [ null ];
		var reverseRuntimeIds = { };
		for (id = 1; true; id++)
		{
			stringIdStr = app.typeIDToStringID (id);
			if (stringIdStr !== "")
			{
				runtimeIds.push (stringIdStr);
			}
			else
			{
				break;
		...
		stringIdStrs = [ ];
		for (index = 1; index < runtimeIds.length; index++)
		{
			stringIdStrs.push (runtimeIds[index]);
		}
		stringIdStrs.sort ();
		for (index = 0; index < stringIdStrs.length; index++)
		{
			stringIdStr = stringIdStrs[index];
			reverseRuntimeIds[stringIdStr] = app.stringIDToTypeID (stringIdStr);
		}
	}
	if (listPredefinedIds)
	{
		var predefinedIds = { };
		var reversePredefinedIds = { };
		for (id0 = 0x20; id0 < 0x7F; id0++)
		{
			for (id1 = 0x20; id1 < 0x7F; id1++)
			{
				for (id2 = 0x20; id2 < 0x7F; id2++)
				{
					for (id3 = 0x20; id3 < 0x7F; id3++)
					{
//						id = (id0 << 24) | (id1 << 16) | (id2 << 8) | id3;
						id = (((((id0 * 256) + id1) * 256) + id2) * 256) + id3;
						stringIdStr = app.typeIDToStringID (id);
						if (stringIdStr !== "")
						{
							charIdStr = "'" + app.typeIDToCharID (id) + "'";
							predefinedIds[charIdStr] = jamEngine.getConflictingStringIdStrs (charIdStr) || stringIdStr;
						}
					}
				}
			}
		...
		stringIdStrs = [ ];
		for (charIdStr in predefinedIds)
		{
			if (predefinedIds.hasOwnProperty (charIdStr))
			{
				stringIdStr = predefinedIds[charIdStr];
				if (typeof stringIdStr === 'string')
				{
					stringIdStrs.push (stringIdStr);
				}
				else
				{
					for (index = 0; index < stringIdStr.length; index++)
					{
						stringIdStrs.push (stringIdStr[index]);
					}
				}
			}
		}
		...
		{
			stringIdStr = stringIdStrs[index];
			reversePredefinedIds[stringIdStr] = "'" + app.typeIDToCharID (app.stringIDToTypeID (stringIdStr)) + "'";
		}
	}
	// Time to finish the job and dump everything out
	var endTime = new Date ();
	var diffTime = Math.floor ((endTime.getTime () - beginTime.getTime ()) / 1000);
	var appName = app.path.fsName;
	var currentTime = new Date ();
	var log =
	{
		"application": appName.substring (appName.lastIndexOf ('/') + 1) + " (" + app.version + ")",
		"date": currentTime.toLocaleString (),
		"elapsedTime": "about " + diffTime + " seconds (" + Math.floor (diffTime / 60) + " minutes " + Math.floor (diffTime % 60) + " seconds)",
	}
	if (listRuntimeIds)
	{
		if (listReverseToo)
		{
			...
			{
				"direct": runtimeIds,
				"reverse": reverseRuntimeIds
			};
		}
		else
		{
			log["runtimeIds"] = runtimeIds;
		}
	}
	if (listPredefinedIds)
	{
		if (listReverseToo)
		{
			log["predefinedIds"] =
			{
				"direct": predefinedIds,
				"reverse": reversePredefinedIds
			};
		}
		else
		{
			log["predefinedIds"] = predefinedIds;
		}
	}
	//
	var logString = jamJSON.stringify (log, (expandTabs) ? 4 : '\t');
	//
	if (appVersion === 8)	// CS
	{
		if ((!defaultFile.exists) || confirm ("The file « " + File.decode (defaultFile.name) + " » already exists. Do you want to overwrite it?"))
		{
			jamUtils.writeTextFile (defaultFile, logString);
			alert ("Internal IDs list file created:\r" + defaultFile.fsName);
		}
	}
	else if (appVersion === 9)	// CS2
	{
		var file = defaultFile.saveDlg (promptString);
		if (file)
		{
			jamUtils.writeTextFile (file, logString);
		}
	}
	else if (appVersion >= 10)	// CS3 or later
	{
